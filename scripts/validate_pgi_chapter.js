...
const chIdx = parseInt(process.argv[2] || '0');
...
allParas.forEach((p, i) => {
  ...
  if (enBullets >= 3 && jpBullets === 0 && jpAbc === 0) {
    bulletsLost++;
    issues.push(`BULLETS_LOST para ${i} [${p.num}]: EN ${enBullets} bullets`);
  }
...
console.log(`=== PGI Ch${chIdx + 1}: ${ch.title} ===`);
...
console.log(`LIST_COUNT:     ${listCount}`);
if (issues.length > 0) {
  console.log('\nIssues:');
  issues.forEach(i => console.log('  ' + i));
}
if (tooShort === 0 && amountMissing === 0 && formatChange === 0 && bulletsLost === 0 && listCount === 0) {
  console.log('\nALL CLEAR');
}
