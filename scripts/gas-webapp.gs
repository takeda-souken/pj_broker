...
const HEADERS = {
  QuizLog: ['timestamp', 'sentAt', 'module', 'topic', 'questionId', 'selected', 'correct', 'isCorrect', 'timeMs', 'mode', 'origSelected', 'origCorrect'],
...
function doPost(e) {
  try {
    ...
    // Auto-expand headers if new columns arrive
    const sheetHeaders = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
    let expanded = false;
    for (const col of headers) {
      if (!sheetHeaders.includes(col)) {
        sheetHeaders.push(col);
        sheet.getRange(1, sheetHeaders.length).setValue(col).setFontWeight('bold');
        expanded = true;
      }
    }
    const activeHeaders = expanded ? sheetHeaders : sheetHeaders;
    ...
    const row = activeHeaders.map(col => {
